...
function putStatements(ans,literal) {
	...
	if (isGrid==true) {
		for (var j=0;j<grid.length ; j++)
		{
			...
			if (document.getElementById("checkGrid"+ans[0].subject)==null) {
				var br= document.createElement("br");
				br.id = "checkGrid"+ans[0].subject;
				document.body.appendChild(br);
				variable = "row1";
			}
			else{
	...
	for (var i=0;i<ans.length ;i++ )
	{
		for (var j=0;j<allRules.length;j++ )
		{
			if((ans[i].rule_id == allRules[j].rule_id)&&(ans[i].object!="")&&(ans[i].object_id=="")){	
				...
				if((validation == "text")||(validation=="")||(validation=="date")||(validation=="(0[1-9]|1[012])/(0[1-9]|[12][0-9]|3[01])/[12][0-9]{3}")||(validation=="textarea")){
					if(validation!="textarea"){
					...
					if ((validation.length>0))
					{
						...
						if ( document.getElementById(ans[i].subject+ans[i].object+str+variable)==null) {
							alert("The option '"+str+"' doesn't exist in database.\nPlease add this option!");
						}
						isGrid=checkIfGrid(ans[i].subject_id);
	...
	if (contStatements1==contStatements)
	{
		...
		removeElement("loading");
		for (var i=0; i<allCollections.length; i++) {
			if (document.getElementById("checkGrid"+allCollections[i].name)!=null) {
				removeElement("checkGrid"+allCollections[i].name);
			}
		}
	}
}
