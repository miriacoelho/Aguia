	...
	for (i = 0; i < allCollections.length; i++) {
		if (allCollections[i].collection_id==collection_id) {
			for (j = 0; j < allRules.length; j++) {
				...
				var search='><img id="input' + allCollections[i].name + allRules[j].object + allRules[j].rule_id + '" align="top" style="cursor:pointer" src="images/icon-search.PNG" onclick="tabSearch(this.id);"/>';
				if (allCollections[i].collection_id == allRules[j].subject_id) {
